...
class RCAB_wt(nn.Module):
    def __init__(self):
        ...
        self.conv_relu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, groups=4),
                                        nn.ReLU())
        self.conv_relu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, groups=4),
                                        nn.ReLU())
        self.conv_relu3 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, groups=4),
                                        nn.ReLU())
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_relu4 = nn.Sequential(nn.Conv2d(64, 16, kernel_size=1, stride=1, padding=0, groups=4),
                                        nn.ReLU())
...
class WSPN(nn.Module):
    def __init__(self, config=str(Path.cwd() / 'config' / 'wspn.yaml'), wavelet='haar'):
        super().__init__()
        ...
        self.conv_relu = nn.Sequential(nn.Conv2d(64, 64 * (2 ** 2), kernel_size=3, stride=1, padding=1, groups=4),
                                       nn.ReLU())
    ...
    def forward(self, x):
        x = self.wt(x, wavelet=self.wavelet)
        x = self.input_wt(x)
        x = self.RGs_wt(x)
        x = self.conv_relu(x)
        x = self.pixel_shuffle(x)
        x = self.output_wt(x)
        x_img = self.iwt(x, wavelet=self.wavelet)
...
def train_loss(y_wc_pre,
               ...
               l_ll=0.01,
               l_ssim=0.1):
    if y_wc_pre.size() != y_wc.size():
        y_wc_pre = F.interpolate(y_wc_pre, size=(y_wc.size()[-2], y_wc.size()[-1]), mode='nearest')
    if y_img_pre.size() != y_img.size():
        y_img_pre = F.interpolate(y_img_pre, size=(y_img.size()[-2], y_img.size()[-1]), mode='nearest')
...
def inference(model=WSPN(),
    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader)) as pbar:
            pbar.set_description(f'WSPN_ReLU Inference {dataset_name} {specimen_name} {mode}')
            for idx, (wf, gt) in enumerate(data_loader):
                ...
                pre_img_before = model.iwt(pre_wc_before, model.wavelet)
                if pre_img_before.size() != gt.size():
                    pre_img_before = F.interpolate(pre_img_before, (gt.size()[-2], gt.size()[-1]), mode='nearest')
                if pre_img_after.size() != gt.size():
                    pre_img_after = F.interpolate(pre_img_after, (gt.size()[-2], gt.size()[-1]), mode='nearest')
...
def train(model=WSPN(),
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_dataloader)) as pbar:
            pbar.set_description(f'WSPN_ReLU Train {dataset_name} {specimen_name} Epoch {epoch + 1} / {epochs}')
            for idx, (wf, gt) in enumerate(train_dataloader):
                wf, gt = wf.cuda(), gt.cuda()
                gt_wc = model.wt(gt, model.wavelet)
