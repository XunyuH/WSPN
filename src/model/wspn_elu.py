...
def inference(model=WSPN(),
    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader)) as pbar:
            pbar.set_description(f'WSPN_ELU Inference {dataset_name} {specimen_name} {mode}')
...
def train(model=WSPN(),
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_dataloader)) as pbar:
            pbar.set_description(f'WSPN_ELU Train {dataset_name} {specimen_name} Epoch {epoch + 1} / {epochs}')
