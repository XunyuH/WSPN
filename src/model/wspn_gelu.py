...
class RCAB_wt(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_gelu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, groups=4),
                                        nn.GELU())
        self.conv_gelu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, groups=4),
                                        nn.GELU())
...
class WSPN(nn.Module):
    def __init__(self, config=str(Path.cwd() / 'config' / 'wspn.yaml'), wavelet='haar'):
        ...
        self.conv_gelu = nn.Sequential(nn.Conv2d(64, 64 * (2 ** 2), kernel_size=3, stride=1, padding=1, groups=4),
                                       nn.GELU())
...
def inference(model=WSPN(),
    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader)) as pbar:
            pbar.set_description(f'WSPN_GELU Inference {dataset_name} {specimen_name} {mode}')
...
def train(model=WSPN(),
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_dataloader)) as pbar:
            pbar.set_description(f'WSPN_GELU Train {dataset_name} {specimen_name} Epoch {epoch + 1} / {epochs}')
